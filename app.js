...
const aspects = {
  form: {
    title: 'Аспект Формы',
    task: 'Собери гармонию. Какая фигура считается совершенной в геометрии?',
    puzzle: `<input type="text" id="answer" placeholder="Твой ответ">`,
    answer: 'круг'
  },
  sound: {
    title: 'Аспект Звука',
    task: 'Музыка рождает эмоции. Что из этого — не музыкальный жанр?',
  ...
  narrative: {
    ...
    task: 'История разрушена. Какое событие должно быть первым?',
    puzzle: `<select id="answer">
  ...
  vision: {
    ...
    task: 'Смотри глубже. Сколько треугольников на картинке?',
    puzzle: `<p>🔺🔺🔺</p>
             <input type="number" id="answer" placeholder="Число">`,
    answer: '3'
  },
  will: {
    title: 'Аспект Воли',
...
function ending(choice) {
  ...
  if (choice === 'chaos') {
    ...
    document.getElementById('ending-text').innerText =
      'Мир распался. Ты стал Проводником хаоса. (Статус: 🌑)';
  } else {
    document.getElementById('ending-title').innerText = 'Ты сохранил Единство';
    document.getElementById('ending-text').innerText =
      'Источник воссиял вновь. Ты стал Хранителем целого. (Статус: ✨)';
  }
}
