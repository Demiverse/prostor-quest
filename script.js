...
function initPlanets() {
  ...
  planets = planetsEls.map((planet, i) => {
    ...
    // базовая позиция задаётся через left/top
...
function animatePlanets() {
  ...
  planets.forEach(p => {
    // небольшой дрейф
    let driftX = Math.sin(tick + p.angle) * 10;
    let driftY = Math.cos(tick + p.angle) * 10;

    // пульсация
    let scale = 1 + Math.sin(tick + p.pulseOffset) * 0.05;

    p.el.style.transform = `translate(${driftX}px, ${driftY}px) scale(${scale})`;
  });
